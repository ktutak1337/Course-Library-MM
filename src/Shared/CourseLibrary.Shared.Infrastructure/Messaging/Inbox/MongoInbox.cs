...
internal sealed class MongoInbox : IInbox
    {
        ...
        public async Task HandleAsync(IMessage message, Func<Task> handler, string module, CancellationToken cancellationToken = default)
        {
            if (!Enabled)
            {
            ...
            if (await collection.AsQueryable().AnyAsync(x => x.Id == message.Id, cancellationToken))
            {
            ...
            try
            {
                ...
                await collection.InsertOneAsync(new InboxMessage
                {
                    ...
                    Timestamp = _clock.CurrentDate().ToUnixTimeMilliseconds()
                });
                if (session is {})
                {
